...
@section('page-title', 'صندوق الوارد')
@section('page-description', 'رسائلك الواردة')
    ...
    <div class="divide-y divide-gray-100">
        @forelse($messages as $message)
            <a href="{{ route('messages.show', $message) }}" 
               class="block p-4 hover:bg-gray-50 transition {{ !$message->read_at ? 'bg-indigo-50' : '' }}">
                <div class="flex items-center gap-4">
                    ...
                    <div class="flex-1 min-w-0">
                        <div class="flex items-center gap-2 mb-1">
                            ...
                            @endif
                            <p class="font-semibold text-gray-800 {{ !$message->read_at ? 'font-bold' : '' }}">
                                {{ $message->sender->name }}
                            </p>
                            <span class="text-xs text-gray-400">{{ $message->created_at->diffForHumans() }}</span>
                        </div>
                        <p class="font-medium text-gray-700 truncate {{ !$message->read_at ? 'font-bold' : '' }}">
                            {{ $message->subject }}
                        </p>
        ...
        @empty
            <div class="p-12 text-center text-gray-500">
                ...
                <h3 class="text-xl font-bold text-gray-600 mb-2">صندوق الوارد فارغ</h3>
