...
{
    /**
     * إنشاء بيانات تجريبية شاملة للوحدات الوظيفية
     * (المالية، المكتبة، النقل، الاختبارات، المواد الدراسية)
     */
    public function run(): void
    {
        ...
        if (!$school) {
            echo "❌ المدرسة التجريبية غير موجودة. شغّل AccountsSeeder أولاً.\n";
            return;
        }
        ...
        if (!$teacher1) {
            echo "❌ لا يوجد معلم. شغّل AccountsSeeder أولاً.\n";
            return;
        }
        ...
        $busesAll = Bus::where('school_id', $sid)->get();
        $routesData = [
        ...
        foreach ($students as $i => $stu) {
            $route = $routes->get($i % $routes->count());
            if ($route) {
        ...
        foreach ($students->take(2) as $stu) {
            ...
            if ($quiz) {
                ...
                $score = $maxScore > 0 ? rand(round($maxScore * 0.6), $maxScore) : 0;
        ...
        foreach ($materials as $i => $data) {
            $classroom = $classrooms->get($i % $classrooms->count()) ?: $classrooms->first();
            $subject = $subjects->get($i % $subjects->count());
